...
void vive_update(float curr_time_vive, float desired_x_pos,float desired_y_pos,float desired_z_pos, float desired_yaw_rate);
void calibrate_vive();
...
int quad_thrust = 1550; // neutral speed
...
float yprevVive = 0;
float zprevVive = 0;
float estimated_x_vive = 0;
float estimated_y_vive = 0;
float estimated_z_vive = 0;
float sumZ=0;
int numtimesz=0;
float zVelocity_Estimate=0;
float average_z=-10;
...
int main (int argc, char *argv[])
{
    ...
    //printf("Time \t PWM 0 \t PWM 1 \t  PWM 2 \t PWM 3 \t Current Pitch \t Quad Thrust \t Joy Pitch\r\n");
    ...
    while(run_program==1)
    {
      ...
      vive_update(ref_time,0,-300,0,0);
      //printf("%f",(curr_time-init_time)/1000.0); 
      //printf("Desired yaw rate: %f \r\n",desired_yaw_rate);
      pid_update(desired_pitch_rate,desired_roll_rate,desired_yaw_rate); // set desired pitch angle and roll angle
...
//printf("%f \r\n",real_yaw_angle);
      ...
      // graphTime=(graphTimerE*1000LL+graphTimerE/1000)-(graphTimerS*1000LL+graphTimerS/1000);
       ...
       //printf("%f \t %d \t %d \t %d \t %d \t %f \t %d \t %f\r\n",(curr_time-init_time)/1000.0, pwm0,pwm1,pwm2,pwm3, real_pitch_angle,quad_thrust,joy_pitch); // Mojojojojo
       //printf("%f \t %f \t %f\r\n",roll_angle_acc,imu_data[0],real_pitch_angle);
      ///////////////////////////////////////////////////////////////////////////
...
void calibrate_vive(){
  for (int i = 0; i<10;i++){
    while (local_p.version == last_vive_version){
      //printf("Version not new!");
      local_p=*position; 
    }
    estimated_y_vive = estimated_y_vive+local_p.y;
    estimated_x_vive = estimated_x_vive+local_p.x;
    estimated_z_vive = estimated_z_vive+local_p.z;

    last_vive_version = local_p.version;
  }
  estimated_x_vive = estimated_x_vive/10.0;
  estimated_y_vive = estimated_y_vive/10.0;
  estimated_z_vive = estimated_z_vive/10.0;
  printf("Completed vive calibration.\r\n");
}

void vive_update(float curr_time_vive,float desired_x_pos,float desired_y_pos,float desired_z_pos,float desired_yaw_pos){
  int Kpvive = 150;
  float kprVive = 0.005;//0.027;//0.04;
  float kdrVive = 0.4;//0.53;
  float kppVive = 0.005 ;//0.02;
  float kdpVive = 0.4;//0.53;
  float kptVive = 0.07;//0.07;
  float kdtVive = 2;//2;
  float kitVive = 0.003;//0.003;
  float Avalue = 0.6;//0.8;
  int zK = 50;
  int zAvalue = 0.9;
  int stay_on = 1;
  ...
  if(local_p.version != last_vive_version){
    ...
    // }
      ...
      estimated_x_vive = estimated_x_vive*Avalue+local_p.x*(1-Avalue); 
      estimated_y_vive = estimated_y_vive*Avalue+local_p.y*(1-Avalue);
      estimated_z_vive = estimated_z_vive*zAvalue+local_p.z*(1-zAvalue);

      desired_yaw_rate = Kpvive*(local_p.yaw - desired_yaw_pos); // set global yaw
      float desired_roll_rate_vive = kprVive*(estimated_x_vive - desired_x_pos) + kdrVive*(estimated_x_vive - xprevVive);
      desired_roll_rate = desired_roll_rate_vive*0.5+joy_roll*0.5; // joy roll is a global variable
      //printf("desired_roll_rate_vive:%f value into kpr:%f value in kdr: %f \r\n", desired_roll_rate_vive,kprVive*(estimated_x_vive - desired_x_pos),kdrVive*(estimated_x_vive - xprevVive));
      float desired_pitch_rate_vive = kppVive*(estimated_y_vive - desired_y_pos) + kdpVive*(estimated_y_vive - yprevVive);
      desired_pitch_rate = -desired_pitch_rate_vive*0.5+joy_pitch*0.5; // joy roll is a global variable

      // Thrust D term sensor fushion
      if(numtimesz!=0){
      average_z = sumZ/numtimesz;
       }
      zVelocity_Estimate = (zVelocity_Estimate+average_z*zK)*zAvalue+(estimated_z_vive-zprevVive)*(1-zAvalue);
      //printf("Average Accel Z vel: %f, Vive Z vel:%f,estZ:%f\r\n",(zVelocity_Estimate+average_z*zK),(estimated_z_vive-zprevVive),estimated_z_vive);
      numtimesz = 0;
      sumZ = 0;


      float desired_thrust_rate = kptVive*(local_p.z-desired_z_pos)-kdtVive*zVelocity_Estimate+kitVive*(local_p.z-desired_z_pos);
      quad_base_thrust=desired_thrust_rate;

      // Thrust D term fusing



      // Update last prev
      xprevVive = estimated_x_vive;
      yprevVive = estimated_y_vive;
      zprevVive = estimated_z_vive;
      //printf("desired_pitch_rate: %f desired_roll_rate: %f \r\n", desired_pitch_rate,desired_roll_rate); 
    }
    ...
    numtimesz++; // increment the z counter
    sumZ = sumZ + (accel_z_calibration-imu_data[5]);
    //printf("Counts for Z: %d, Sum of Z: %f")
...
void pid_update(float desired_pitch, float desired_roll,float desired_yaw){
  // PID control values
  float Kp = 15;//15; //7;
  float Kd = 300;//300; //120;
  float Ki = 0.01;//0.05; //0.003;
  float Kpr = 15;//best: 6.5;//8;
  float Kdr = 300;//best: 90;//200;
  float Kir = 0.01;//best: 0.005;//0.02;
  float Kpy=1;//1;
  float Kdy=0;
  float Kiy=0;
  ...
  pitch_d_error = real_pitch_angle - prev_pitch; //prev_error - pitch_error;
  ...
  //printf("%f \t %f \t %f \t %f \t %f\r\n", roll_angle_acc, real_pitch_angle, imu_data[0], pitch_error, pitch_d_error);
  ///////////////////////////// MILESTONE for WEEK 4 /////////////////////////////

  // calculate but do nothing with them at the moment.
  roll_error = desired_roll - real_roll_angle;
  roll_d_error = real_roll_angle-prev_roll;//prev_d_error - roll_error;
  ...
  //yaw error
  yaw_error = desired_yaw - real_yaw_angle;
  ...
  //
  //
   pwm0 = quad_thrust -yaw_error*Kpy -pitch_error*Kp + pitch_d_error*Kd - pitch_i_error*Ki - roll_error*Kpr + roll_d_error*Kdr - roll_i_error*Kir;
   pwm1 = quad_thrust +yaw_error*Kpy+ pitch_error*Kp - pitch_d_error*Kd + pitch_i_error*Ki - roll_error*Kpr + roll_d_error*Kdr - roll_i_error*Kir;
   pwm2 = quad_thrust +yaw_error*Kpy- pitch_error*Kp + pitch_d_error*Kd - pitch_i_error*Ki + roll_error*Kpr - roll_d_error*Kdr + roll_i_error*Kir;
   pwm3 = quad_thrust -yaw_error*Kpy+ pitch_error*Kp - pitch_d_error*Kd + pitch_i_error*Ki + roll_error*Kpr - roll_d_error*Kdr + roll_i_error*Kir;

  //pwm0 = quad_thrust - yaw_error*Kpy;
  //pwm1 = quad_thrust + yaw_error*Kpy;
...
//
  ...
  prev_pitch=real_pitch_angle;
  prev_roll=real_roll_angle;
}
...
void get_joystick(Keyboard keyboard){
  // printf("%c\n\r",keyboard.keypress);
  // printf("curr:%d last:%d\n\r",keyboard.version,last_version);
  signal(SIGINT, &trap); // instant interrupt!
  if (keyboard.version != last_version){
      ...
      if (start_pause || keyboard.keypress == '!'){
        printf("Paused!"); // do pause
        signal(SIGINT, &trap); // instant interrupt!
        set_PWM(0,1000);
        set_PWM(1,1000);
        set_PWM(2,1000);
        ...
        while(keyboard.keypress!='\"'){
          keyboard=*shared_memory;
          last_version = keyboard.version;
          printf("Paused!\r\n"); // do pause
          if(keyboard.keypress=='#'){
            printf("Y pressed! Calibrating... "); // do calibration
            calibrate_imu();  
          }
        }
        start_pause=0;
        printf("X pressed! Unpaused!"); // do unpause
      }
      ...
      //MOJOJO TIME
      joy_thrust = -2*(keyboard.thrust-128); // update thrust // up is 16
      joy_yaw = 60*(keyboard.yaw-128)/128; // update yaw // right is high
      joy_roll = -10*(keyboard.roll-128)/128; //  update roll // right is high
      joy_pitch = 10*(keyboard.pitch-128)/128; // update pitch // up is 16
...
void calibrate_imu()
{
  ...
  for (int i = 0; i<1000; i++){
    ...
    myaverage6 = myaverage6 + imu_data[5]; // our acc z
