...
int quad_thrust = 1600; // neutral speed
...
void pid_update(float desired_pitch, float desired_roll,float desired_yaw){
  ...
  printf("%f %d %d %d %d\r\n",real_yaw_angle,pwm0,pwm1,pwm2,pwm3);
...
void get_joystick(Keyboard keyboard){
  ...
  if (keyboard.version != last_version){
      ...
      if (keyboard.keypress =='\"'){
        ...
        printf("B pressed! Paused!"); // do pause
      }
      else if(keyboard.keypress == '#' ){
	...
	calibrate_imu(); 
      ...
      joy_yaw = 20*(keyboard.yaw-128)/128; // update yaw // right is high
      joy_roll = -20*(keyboard.roll-128)/128; //  update roll // right is high
